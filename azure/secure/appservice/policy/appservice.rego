...
sapp_authentication {
	count(d.sapp_total) == count(d.sapp_auth)
}
deny[msg] {                                                                 
	sapp_failed := d.sapp_total - d.sapp_auth
    count(sapp_failed) != 0
...
web_app_https_only {
	count(d.sapp_total) == count(d.web_app_https)
}
deny[msg] {                                                                 
...
web_app_tls_version {
	count(d.sapp_total) == count(d.web_app_latest_tls) + count(d.web_app_latest_default)
}
deny[msg] {         
	tls= d.web_app_latest_tls | d.web_app_latest_default                                                
	sapp_failed := d.sapp_total - tls
    count(sapp_failed) != 0
...
web_app_client_cert_enabled {
	count(d.sapp_total) == count(d.web_app_client_cert)
}
deny[msg] {                                                                 
...
web_app_aad_enabled {
	count(d.sapp_total) == count(d.web_app_aad)
}
deny[msg] {                                                                 
...
web_app_php_version {
	...
	count(d.sapp_total) == count(d.web_app_latest_php)
}
deny[msg] {
	resource := input.resource_changes[i]
	resource.type == "azurerm_app_service"
...
web_app_python_version {
	...
	count(d.sapp_total) == count(d.web_app_latest_python)
}
deny[msg] {
	resource := input.resource_changes[i]
	resource.type == "azurerm_app_service"
...
web_app_java_version {
	...
	count(d.sapp_total) == count(d.web_app_latest_java)
}
deny[msg] {  
	resource := input.resource_changes[i]
	resource.type == "azurerm_app_service"
...
web_app_http_version {
	count(d.sapp_total) == count(d.web_app_latest_http)
}
deny[msg] {                                                              
...
web_app_ftp_disable {
	count(d.sapp_total) == count(d.web_app_ftp)
}
deny[msg] {                                                                 
